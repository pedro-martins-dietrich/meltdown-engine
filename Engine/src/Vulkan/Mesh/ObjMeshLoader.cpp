...
void mtd::ObjMeshLoader::loadMaterial(std::string filePath, Material& material)
{
...
void mtd::ObjMeshLoader::loadMaterials
(
	std::string filePath,
	std::vector<Material>& materials,
	std::unordered_map<std::string, uint32_t>& materialIDs,
...
)
{
	...
	while(std::getline(file, line))
	{
		...
		{
			float indexOfRefraction = std::stof(words[1]);
			materials[currentMaterialID].addFloatData(MaterialFloatDataType::IndexOfRefraction, &indexOfRefraction);
		}
		else if(!words[0].compare("Pr"))
		{
			float roughness = std::stof(words[1]);
...
size_t mtd::ObjMeshLoader::readFaceData(const std::vector<std::string>& words, ObjData& data)
{
...
void mtd::ObjMeshLoader::readVertex(const std::string& vertexDescription, ObjData& data)
{
